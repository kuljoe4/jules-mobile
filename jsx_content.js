...
const ACT_POLL_KEY    = "jac_act_poll_ms";
const ACTIVE_POLL_KEY = "jac_active_poll_ms";
...
function loadActPollMs() {
  try { const v=parseInt(localStorage.getItem(ACT_POLL_KEY)); return POLL_OPTIONS.some(o=>o.ms===v)?v:DEFAULT_POLL_MS; } catch { return DEFAULT_POLL_MS; }
}
function saveActPollMs(ms) { try { localStorage.setItem(ACT_POLL_KEY,ms); } catch {} }

function loadActivePollMs() {
  try { const v=parseInt(localStorage.getItem(ACTIVE_POLL_KEY)); return POLL_OPTIONS.some(o=>o.ms===v)?v:DEFAULT_POLL_MS; } catch { return DEFAULT_POLL_MS; }
}
function saveActivePollMs(ms) { try { localStorage.setItem(ACTIVE_POLL_KEY,ms); } catch {} }
...
const CACHE_LIMIT_KEY = "jac_cache_limit";
const DEFAULT_CACHE_LIMIT = 5;

function loadCacheLimit() {
  try { const v = parseInt(localStorage.getItem(CACHE_LIMIT_KEY)); return isNaN(v) ? DEFAULT_CACHE_LIMIT : v; } catch { return DEFAULT_CACHE_LIMIT; }
}
function saveCacheLimit(v) { try { localStorage.setItem(CACHE_LIMIT_KEY, v); } catch {} }
...
const ACTIVITY_LIMIT_KEY = "jac_activity_limit";
const DEFAULT_ACTIVITY_LIMIT = 100;

function loadActivityLimit() {
  try { const v = parseInt(localStorage.getItem(ACTIVITY_LIMIT_KEY)); return ACTIVITY_LIMIT_OPTIONS.some(o=>o.val===v)?v:DEFAULT_ACTIVITY_LIMIT; } catch { return DEFAULT_ACTIVITY_LIMIT; }
}
function saveActivityLimit(v) { try { localStorage.setItem(ACTIVITY_LIMIT_KEY,v); } catch {} }
...
const CUSTOM_DAILY_KEY = "jac_custom_daily";
...
function loadCustomDaily() { try { return parseInt(localStorage.getItem(CUSTOM_DAILY_KEY))||50; } catch { return 50; } }
function saveCustomDaily(v) { try { localStorage.setItem(CUSTOM_DAILY_KEY,v); } catch {} }
...
const PERSONA_PROMPTS_KEY = "jac_persona_prompts";

function loadPersonas() {
  try {
    const saved = JSON.parse(localStorage.getItem(PERSONA_PROMPTS_KEY) || "{}");
...
function savePersonaPrompt(id, prompt) {
  try {
    const saved = JSON.parse(localStorage.getItem(PERSONA_PROMPTS_KEY) || "{}");
    saved[id] = prompt;
    localStorage.setItem(PERSONA_PROMPTS_KEY, JSON.stringify(saved));
  } catch {}
}

function resetPersonas() {
  try {
    localStorage.removeItem(PERSONA_PROMPTS_KEY);
...
const T = {
  ...
  indigo:"#a5b4fc", indigoDim:"rgba(165,180,252,0.15)",
  amber:"#fcd34d", amberDim:"rgba(252,211,77,0.15)",
  red:"#fb7185",   redDim:"rgba(251,113,133,0.15)",
  blue:"#60a5fa",  blueDim:"rgba(96,165,250,0.15)",
  purple:"#d8b4fe",purpleDim:"rgba(216,180,254,0.15)",
...
const STATUS_META = {
  ...
  PLANNING:              { label:"PLANNING", color:T.purple,    bg:T.purpleDim,             pulse:true,  icon:"plan" },
  AWAITING_PLAN_APPROVAL:{ label:"APPROVE",  color:T.indigo,    bg:T.indigoDim,             pulse:true,  icon:"approve" },
  AWAITING_USER_FEEDBACK:{ label:"INPUT",    color:T.amber,     bg:T.amberDim,              pulse:true,  icon:"tasks" },
  ...
  COMPLETED:             { label:"DONE",     color:T.brand,     bg:T.brandDim,              pulse:false, icon:"check" },
  FAILED:                { label:"FAILED",   color:T.red,      bg:T.redDim,                pulse:false, icon:"x" },
};

// ─── Network tracker ──────────────────────────────────────────────────────────
function makeNetTracker() {
  const STATS_KEY = "jac_net_stats";
  const initial = (() => {
    try { return JSON.parse(localStorage.getItem(STATS_KEY)) || { overall: { in: 0, out: 0 }, daily: {} }; }
    catch { return { overall: { in: 0, out: 0 }, daily: {} }; }
  })();
  ...
  const getUsage = () => {
    ...
    return { today, month, overall: stats.overall };
  };
  ...
  return {
    record(label, bytesIn, bytesOut, status) {
      ...
      // Keep only last 60 days of daily stats to prevent bloat
      const dailyKeys = Object.keys(stats.daily).sort();
      if (dailyKeys.length > 60) delete stats.daily[dailyKeys[0]];
...
async function apiCall(apiKey, path, opts={}) {
  const url = `${BASE}${path}`;
  const headers = { "x-goog-api-key":apiKey, "Content-Type":"application/json", ...(opts.headers||{}) };
...
const fmtChars = n => n<1000?`${n}c`: `${(n/1000).toFixed(1)}kc`;
const getPR = (s) => {
  if (!s) return null;
  const findInOutputs = (outputs) => {
    if (!outputs || !Array.isArray(outputs)) return null;
    const prObj = outputs.find(o => o.githubPullRequest)?.githubPullRequest;
    if (prObj) return prObj;
    // Some versions put the URL directly in a string or as a nested field
    for (const o of outputs) {
      if (o.pullRequestUrl || o.prUrl) return { url: o.pullRequestUrl || o.prUrl, number: (o.pullRequestUrl || o.prUrl).split("/").pop() };
    }
    return null;
  };

  // 1. Standard locations
  let pr = findInOutputs(s.outputs) || s.githubPullRequest || s.pullRequest || s.sourceContext?.githubRepoContext?.pullRequest || s.github_pull_request;
  if (pr && typeof pr === 'object' && pr.url) return { ...pr, number: pr.number || pr.url.split("/").pop() };

  // 2. Scan all string fields for GitHub PR links (last resort)
  const str = JSON.stringify(s);
  const m = str.match(/https:\/\/github\.com\/[^\/]+\/[^\/]+\/pull\/(\d+)/);
  if (m) return { url: m[0], number: m[1] };

  return null;
};

const getPRInfo = (s, activities = []) => {
  const pr = getPR(s);
  if (!pr) return null;

  let state = "open";
  // Deep scan activities for merge or close events
  for (const a of activities) {
    const desc = (a.progressUpdated?.description || "").toLowerCase();
    const title = (a.progressUpdated?.title || "").toLowerCase();
    if (desc.includes("merged pull request") || title.includes("merged pull request") || a.sessionCompleted) {
      state = "merged";
      break;
    }
    if (desc.includes("closed pull request") || title.includes("closed pull request")) {
      state = "closed";
    }
  }
...
const getCheckStatus = (activities = []) => {
  let status = null; // { state: 'pending'|'success'|'failure', label: string, url?: string }
  for (const a of activities) {
    const desc = (a.progressUpdated?.description || "").toLowerCase();
    const title = (a.progressUpdated?.title || "").toLowerCase();
    ...
    if (isSignal) {
      if (desc.includes("passed") || desc.includes("success") || desc.includes("completed") || desc.includes("verified")) {
        status = { state: "success", label: desc.includes("deploy") ? "DEPLOYED" : "CHECKS PASSED" };
      } else if (desc.includes("failed") || desc.includes("failure") || desc.includes("error")) {
        status = { state: "failure", label: desc.includes("deploy") ? "DEPLOY FAILED" : "CHECKS FAILED" };
      } else if (desc.includes("running") || desc.includes("pending") || desc.includes("started") || desc.includes("progress")) {
        status = { state: "pending", label: desc.includes("deploy") ? "DEPLOYING" : "CHECKS RUNNING" };
      }

      const urlMatch = (a.progressUpdated?.description || "").match(/https:\/\/github\.com\/[^\/]+\/[^\/]+\/(?:actions\/runs|deployments|actions)\/\d*/);
      if (urlMatch && status) status.url = urlMatch[0];
    }
  }
  return status;
};

const getBranchInfo = (s, activities = []) => {
  if (!s) return null;
  const context = s.sourceContext?.githubRepoContext;
  const base = context?.startingBranch || "main";

  let working = null;
  // Scan activities for "pushed to <branch>" or similar
  for (const a of activities) {
    const desc = a.progressUpdated?.description || "";
    // Match common Jules patterns for branch names, skipping filler words
    const m = desc.match(/push(?:ing|ed).* to (?:the |a )?([^\s\.\(\)\`]+)/i) ||
              desc.match(/branch (?:named )?([^\s\.\(\)\`]+)/i) ||
              desc.match(/created.* branch ([^\s\.\(\)\`]+)/i);
    if (m) {
      const bname = m[1].replace(/['"`\.]/g, "");
      if (bname !== "main" && bname !== "master") working = bname;
    }
  }

  // Fallback: search for tree links which often contain the branch
  if (!working) {
    const str = JSON.stringify(s) + JSON.stringify(activities);
    // Be more specific with tree links to avoid matching repo names or other segments
    const m = str.match(/https:\/\/github\.com\/[^\/]+\/[^\/]+\/tree\/([a-zA-Z0-9\-_]+)/);
    if (m) working = m[1];

    // Also look for branch links in descriptions or compare links
    if (!working) {
      const bm = str.match(/https:\/\/github\.com\/[^\/]+\/[^\/]+\/branches\/([a-zA-Z0-9\-_]+)/) ||
                 str.match(/https:\/\/github\.com\/[^\/]+\/[^\/]+\/compare\/[^\.]+\.\.\.([a-zA-Z0-9\-_]+)/);
      if (bm) working = bm[1];
    }
  }
  ...
  // Fallback: extract from PR if found
  ...
  return { base, working: working || base, repo: repoUrl ? repo : null, repoUrl, isNew: working && working !== base };
};

const getAheadCount = (activities = []) => {
  let count = 0;
  // Use a Set to avoid double counting if activities repeat or describe the same push
  const pushHashes = new Set();

  for (const a of activities) {
    const desc = (a.progressUpdated?.description || "").toLowerCase();

    // Extract commit hashes if present to be more precise (7+ chars for short hash, 40 for full)
    const hashes = (a.progressUpdated?.description || "").match(/\b[a-f0-9]{7,40}\b/g);
    if (hashes) {
      hashes.forEach(h => {
        // Try to filter out things that look like hashes but aren't (e.g. IDs)
        if (!h.includes("-") && h.length >= 7) pushHashes.add(h);
      });
    }

    // Match patterns like "pushed 3 commits", "committed 2 changes", etc.
    const m = desc.match(/(?:pushed|committed|applied)\s+(\d+)\s+(?:commit|change|fix|update)/i);
    if (m) {
      if (!hashes) count += parseInt(m[1], 10);
...
const PWABanner = () => {
  ...
  if (isFile) return (
    <div style={{
      ...
      <button onClick={()=>{sessionStorage.setItem("pwa_dismissed","1");setDismissed(true);}} style={{background:"none",border:"none",cursor:"pointer",color:"#4a5e76",fontSize:18,flexShrink:0,padding:0,lineHeight:1}}>×</button>
    </div>
  );
  ...
  if (canInstall) return (
    <div style={{
      ...
      <button onClick={()=>{sessionStorage.setItem("pwa_dismissed","1");setDismissed(true);}} style={{background:"none",border:"none",cursor:"pointer",color:"#4a5e76",fontSize:18,padding:0,lineHeight:1,flexShrink:0}}>×</button>
    </div>
  );
...
const Bar = ({pct,status}) => {
  ...
  return (
    <div style={{height:3,background:T.line,borderRadius:2,overflow:"hidden"}}>
      <div style={{height:"100%",width:`${pct}%`,borderRadius:2,background:c,boxShadow:`0 0 8px ${c}60`,transition:"width .5s ease",transform:"translateZ(0)"}}/>
    </div>
  );
};

const Btn = ({children,onClick,color=T.brand,disabled=false,outline=false,sm=false,style:s={}}) => (
...
const PickerBtn = ({ label, isAct, onClick, activeColor=T.brand, activeBg=null }) => (
  <button onClick={onClick} style={{
    ...
    outline:`1px solid ${isAct ? `${activeColor}60` : T.border}`,
    color:isAct ? activeColor : T.muted,
    fontFamily:"'JetBrains Mono',monospace", fontSize:9, fontWeight:isAct?700:400,
...
const ExpandableContent = memo(({ text, limit = 300, showCopy = false }) => {
  ...
  useEffect(() => {
    if (ref.current && ref.current.scrollHeight > limit) setShowToggle(true);
  }, [text, limit]);
          ...
          <>
            ...
            <button
              ...
              style={{
                ...
                padding: 0, display: "flex", alignItems:"center", gap: 4, transition: "color .2s"
              }}
            >
    ...
    if (i % 2 === 1) { // Code block
      ...
      return (
        <div key={i} style={{
          ...
          {lang && (
            <div style={{padding:"4px 10px", background:T.surface, borderBottom:`1px solid ${T.border}`, fontSize:11, color:T.textDim, textTransform:"uppercase", letterSpacing:"0.05em"}}>
              {lang}
            </div>
          )}
...
const MediaArtifacts = memo(({ artifacts, ts, onMediaClick }) => {
  ...
  return (
    <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:8 }}>
      {mediaItems.map((a, i) => {
        const isVideo = (a.media.mimeType || "").includes("video") || (a.media.mimeType || "").includes("webm");
        return (
          <button
            key={i}
            onClick={() => onMediaClick?.({ ...a.media, ts })}
            style={{
          ...
          >
            {isVideo ? (
              <video
                src={`data:${a.media.mimeType};base64,${a.media.data}`}
...
// ─── Chat Bubble ──────────────────────────────────────────────────────────────
const ChatBubble = memo(({ act, type, onMediaClick, onEdit }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background:isUser?T.blueDim:T.brand,
        border:`1px solid ${isUser?T.blue+"40":T.brandLight+"40"}`,
      ...
      <div style={{
        ...
        border:`1px solid ${isUser?T.blue+"40":T.brandDark+"35"}`,
        ...
        }}><ExpandableContent text={text} limit={400}/></div>
        <MediaArtifacts artifacts={act.artifacts} ts={act.createTime} onMediaClick={onMediaClick}/>
        <div style={{
          marginTop:5,fontFamily:"'JetBrains Mono',monospace",fontSize:10,
          color:isUser?T.blue+"70":T.brand+"60",textAlign:isUser?"right":"left",
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  color:copied?T.brand:T.muted, fontWeight:800, fontSize:9, letterSpacing:"0.05em",
                  display:"flex", alignItems:"center", gap:3, transition: "color .2s"
                }}
              >
                <Ic n="copy" s={10} c={copied?T.brand:T.muted}/>
                {copied?"COPIED":"COPY"}
              </button>
            </>
          )}
          {isUser && !isTemp && onEdit && (
            <>
              ...
              <button
                ...
                style={{
                  background:"none", border:"none", padding:"0 2px", cursor:"pointer",
                  color:T.brand, fontWeight:800, fontSize:9, letterSpacing:"0.05em"
                }}
              >EDIT</button>
            </>
          )}
...
const TimelineEvent = memo(({ act, onMediaClick }) => {
  ...
  switch (type) {
          ...
          {(() => {
            ...
            if (prMatch) return (
              <div style={{marginTop:12}}>
                <a href={prMatch[0]} target="_blank" rel="noopener" style={{
            ...
            if (commitMatch) return (
              <div style={{marginTop:12}}>
                <a href={commitMatch[0]} target="_blank" rel="noopener" style={{
                  display:"inline-flex", alignItems:"center", gap:6, padding:"6px 12px",
            ...
            if (branchMatch) return (
              <div style={{marginTop:12}}>
                <a href={branchMatch[0]} target="_blank" rel="noopener" style={{
                  display:"inline-flex", alignItems:"center", gap:6, padding:"6px 12px",
                  background:T.blueDim, border:`1px solid ${T.blue}40`, borderRadius:6,
                  color:T.blue, textDecoration:"none", fontFamily:"'JetBrains Mono',monospace",
                  fontSize:12, fontWeight:800
    ...
    case "sessionCompleted":
      ...
      const pr = act.sessionCompleted?.outputs?.find(o => o.githubPullRequest)?.githubPullRequest;
      detail = (
        <div style={{marginTop:8, display:"flex", gap:10, flexWrap:"wrap"}}>
          {pr && (
            <a href={pr.url} target="_blank" rel="noopener" style={{
...
const MediaModal = ({ media, onClose }) => {
  ...
  const handleDownload = (e) => {
    ...
    link.download = `jules-artifact-${Date.now()}.${media.mimeType.split("/")[1] || "png"}`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const isVideo = (media.mimeType || "").includes("video") || (media.mimeType || "").includes("webm");
      ...
      <div style={{
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:11, color:T.textDim }}>
            {media.mimeType} · {fmtTime(new Date(media.ts).getTime())}
          </div>
        </div>
        <button
...
const ActivityFeed = memo(({ activities, showAll, onShowAll, onMediaClick, onEditMessage }) => {
    ...
    <>
      ...
      {deduped.map(act => {
        ...
        if (type === "userMessaged" || type === "agentMessaged") {
          return <ChatBubble key={key} act={act} type={type} onMediaClick={onMediaClick} onEdit={onEditMessage}/>;
        }
        return <TimelineEvent key={key} act={act} onMediaClick={onMediaClick}/>;
      })}
    </>
  );
});
...
const PlanView = memo(({ activities, session, apiKey, onApprove, onSendFeedback, busy }) => {
  ...
  const approved = useMemo(() => {
    ...
    const planTs = new Date(planAct.createTime).getTime();
    return activities.some(a => a.planApproved && new Date(a.createTime).getTime() >= planTs);
  }, [activities, planAct]);
...
const DiffViewer = ({ patch }) => {
  ...
  const groups = useMemo(() => {
    ...
    lines.forEach((line) => {
      ...
      if (line.startsWith("+++ ")) {
        ...
        currentGroup = { file, hunks: [], adds: 0, rems: 0 };
  ...
  if (!patch || groups.length === 0) return (
  ...
  const safeCollapsed = collapsed || new Set();
    ...
    <div style={{display:"flex", flexDirection:"column", gap:16}}>
      <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 4px"}}>
        <div style={{display:"flex", alignItems:"center", gap:12}}>
          <div style={{fontFamily:"'JetBrains Mono',monospace", fontSize:11, fontWeight:700, color:T.textDim, letterSpacing:"0.05em"}}>
            {groups.length} FILE{groups.length!==1?'S':''} CHANGED
          </div>
        ...
        <div style={{display:"flex", gap:8}}>
          ...
          <PickerBtn label="COLLAPSE ALL" isAct={safeCollapsed.size === groups.length} onClick={() => toggleAll(true)} />
        </div>
      </div>
      ...
      {groups.map((g, gi) => {
        ...
        return (
          <div key={gi} style={{background:"#040507", border:`1px solid ${T.border}`, borderRadius:8, overflow:"hidden"}}>
            <div
              role="button"
              tabIndex={0}
              ...
              style={{
                padding:"8px 12px", background:T.surface, borderBottom:isCollapsed? "none" : `1px solid ${T.border}`,
                display:"flex", alignItems:"center", gap:8, cursor:"pointer", userSelect:"none", outline:"none"
              }}
            >
              <Ic n={isCollapsed ? "chevron_right" : "chevron_down"} s={16} c={T.muted}/>
              <Ic n="code" s={14} c={T.brand}/>
              <span style={{fontFamily:"'JetBrains Mono',monospace", fontSize:13, fontWeight:700, color:T.text, flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>{g.file}</span>
              <div style={{display:"flex", alignItems:"center", gap:6, fontFamily:"'JetBrains Mono',monospace", fontSize:10, fontWeight:800, opacity:0.8}}>
                <span style={{color:T.brand}}>+{g.adds}</span>
                <span style={{color:T.red}}>-{g.rems}</span>
              </div>
            </div>
            ...
            <div style={{
              ...
              {g.hunks.map((h, hi) => (
                <div key={hi}>
                  <div style={{padding:"4px 12px", background:"rgba(77,166,255,.05)", color:T.blue, fontSize:12, borderBottom:`1px solid ${T.border}40`}}>
                    {h.header}
                  </div>
                  {h.lines.map((line, li) => {
                    const isAdd = line.startsWith("+");
                    const isRem = line.startsWith("-");
                    const c = isAdd ? T.brand : isRem ? T.red : T.muted;
                    const bg = isAdd ? "rgba(0,232,122,.05)" : isRem ? "rgba(255,68,85,.05)" : "transparent";
                    const symbol = isAdd ? "+" : isRem ? "-" : " ";
                    const content = (isAdd || isRem) ? line.slice(1) : line;

                    return (
                      <div key={li} style={{display:"flex", background:bg, minHeight:20, borderLeft:`2px solid ${isAdd?T.brand+'40':isRem?T.red+'40':'transparent'}`}}>
                        <div style={{width:22, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", color:c, opacity:0.8, fontSize:12, userSelect:"none", fontWeight:700, fontFamily:"monospace"}}>
                          {symbol}
                        </div>
                        <div style={{padding:"2px 10px", color:isAdd?T.brand:isRem?T.red:T.text, whiteSpace:"pre-wrap", wordBreak:"break-all", flex:1, opacity: (isAdd||isRem)?1:0.7}}>
                          {content || " "}
                        </div>
                      </div>
                    );
      ...
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <div style={{fontFamily:"'IBM Plex Sans',sans-serif",fontSize:15,color:T.textDim,lineHeight:1.6}}>
            ...
            <a href="https://jules.google.com" target="_blank" rel="noopener" style={{color:T.brandLight,fontFamily:"'JetBrains Mono',monospace",fontSize:13,textDecoration:"none",borderBottom:`1px solid ${T.brandDark}40`}}>jules.google.com → Settings → API</a>
          </div>
        </div>
        <input
          ...
          placeholder="AIza..."
          style={{...inputSt,fontSize:16,fontFamily:"'JetBrains Mono',monospace",marginBottom:10}}
          onKeyDown={e=>e.key==="Enter"&&doConnect()}
          autoFocus
        />
        {err&&<div style={{padding:"8px 12px",borderRadius:5,marginBottom:10,background:T.redDim,border:`1px solid ${T.red}40`,fontFamily:"'JetBrains Mono',monospace",fontSize:12,color:T.red}}>{err}</div>}
        <div style={{display:"flex", gap:10, marginBottom:16}}>
          <Btn onClick={doConnect} disabled={!key.trim()||testing} style={{flex:1}}>
            <Ic n="key" s={12} c="#000"/>
...
// ─── Session Card ─────────────────────────────────────────────────────────────
const SessionCard = memo(({ s, onPress, isSelected, index, activities = [], stats, lastReadTs }) => {
  ...
  const isActuallyDone = activities.some(a => a.sessionCompleted);
  ...
  const activityStats = useMemo(() => {
    ...
    const rawData = JSON.stringify(activities);
  ...
  const bg = isSelected
    ? `linear-gradient(135deg, ${T.brand}15, ${T.bg} 80%), radial-gradient(circle at top right, ${T.brand}10, transparent 60%)`
  ...
  return (
    <button ref={cardRef} onClick={onPress}
      ...
      style={{
        ...
        outline:"none",
        boxShadow: isSelected ? `0 12px 32px -8px ${T.brand}35, inset 0 0 10px ${T.brand}10` : "none",
    ...
    >
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
        ...
        <div
          ...
          style={{display:"flex",alignItems:"center",justifyContent:"center",width:18,height:18,borderRadius:4,background:m.bg,flexShrink:0}}
        >
        ...
        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:T.muted,fontWeight:500,flexShrink:0}}>
          {fmtAgo(new Date(s.updateTime||s.createTime).getTime())}
        </div>
      </div>
      ...
      <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:4, marginLeft:26, flexWrap:"wrap"}}>
        ...
        {(!pri || pri.state === "closed") && b?.isNew && (
          <div style={{display:"flex", alignItems:"center", gap:4, color:T.blue, opacity:0.9}}>
            ...
            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,fontWeight:900}}>{b.working}{ahead>0?` +${ahead}`:''}</span>
          </div>
        )}
        {checks && (
          <div style={{display:"flex", alignItems:"center", gap:3, color:checks.state==="success"?T.brand:checks.state==="failure"?T.red:T.amber, opacity:0.9}}>
            <Ic n={checks.state==="success"?"check":checks.state==="failure"?"x":"refresh"} s={9} c={checks.state==="success"?T.brand:checks.state==="failure"?T.red:T.amber}/>
            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:8,fontWeight:900}}>{checks.label}</span>
          </div>
        )}
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginTop:0,marginLeft:26,fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:T.dim,opacity:0.6}}>
        <div style={{
          ...
          transition: flash ? "none" : "background 5s ease",
...
// ─── Session Detail ───────────────────────────────────────────────────────────
const SessionDetail = ({ session:initSession, apiKey, personas, onBack, onDelete, onSessionUpdate, onStatsUpdate, isDesktop, pollInterval, setPollInterval, isArchived, onArchive, onUnarchive, cacheLimit, activityLimit }) => {
  const [session,setSession]     = useState(initSession);
  const [activities,setActivities] = useState(() => {
    try {
      const cache = JSON.parse(localStorage.getItem(SESSION_CACHE_KEY)) || {};
  ...
  const [syncStats, setSyncPhaseStats] = useState({ count: 0, pages: 0 });
  const [showAll,setShowAll]     = useState(false);
  const [tab,setTab]             = useState("activity");
  const [chatFilter, setChatFilter] = useState("ALL");
  const [msg,setMsg]             = useState(() => {
    try { return localStorage.getItem(`jac_draft_${initSession.id}`) || ""; }
    catch { return ""; }
  });
  ...
  const [scrolled, handleScrollBase] = useScrollThreshold(10);
  ...
  const handleScroll = useCallback(e => {
    ...
    // 1. Always show if near bottom (within 80px - more strict for stability)
    // 2. Hide if scrolling down deep (> 300px)
    // 3. Show if scrolling up
    if (h - y - vh < 80) {
      setComposerVisible(true);
    } else if (y > 300) {
      if (scrollDir.current === 1) setComposerVisible(false);
      else if (scrollDir.current === -1) setComposerVisible(true);
    } else {
      setComposerVisible(true);
    }
  ...
  const loadActivities = useCallback(async (sinceTs=null) => {
    setIsSyncing(true);
    setSyncProgress(sinceTs ? "FETCH" : "CACHE");
    setSyncPhaseStats({ count: 0, pages: 0 });
    let currentPages = 0;
    let currentCount = 0;
    try {
      let pageToken = null;
      ...
      do {
        ...
        const qs = sinceTs
          ? `createTime=${encodeURIComponent(sinceTs)}&pageSize=${pageSize}`
        ...
        setSyncPhaseStats({ count: currentCount, pages: currentPages });
        for (const act of incoming) {
          ...
          if (!existing || JSON.stringify(existing) !== JSON.stringify(act)) {
      ...
      if (changed) {
        ...
        try {
          ...
          const allStats = JSON.parse(localStorage.getItem(ACT_STATS_KEY) || "{}");
          allStats[session.id] = stats;
          localStorage.setItem(ACT_STATS_KEY, JSON.stringify(allStats));
      ...
      if (cacheLimit > 0) {
        try {
          const cache = JSON.parse(localStorage.getItem(SESSION_CACHE_KEY)) || {};
          cache[session.id] = { activities: Array.from(actMapRef.current.values()).filter(a => !a._temp).sort((a,b) => new Date(a.createTime) - new Date(b.createTime)).slice(-activityLimit), ts: Date.now() };
          // Strict LRU Strategy
  ...
  const countdown = useAutoPoll(pollInterval, useCallback(() => Promise.all([loadSession(), loadActivities(lastTsRef.current)]), [loadSession, loadActivities]));
  ...
  useEffect(() => {
    ...
    draftRef.current = setTimeout(() => {
      try {
        if (msg.trim()) localStorage.setItem(`jac_draft_${session.id}`, msg);
        else localStorage.removeItem(`jac_draft_${session.id}`);
      } catch {}
    }, 400);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      try { localStorage.removeItem(`jac_draft_${session.id}`); } catch {}
  ...
  useEffect(() => {
    ...
    if (el) {
      ...
      // Use requestAnimationFrame to avoid forced synchronous layout during effect
  ...
  const handleSendFeedback = useCallback(async (prompt, stayOnTab = false) => {
    ...
    try {
      ...
      actMapRef.current.delete(tempKey);
      if (stayOnTab !== true) setTab("activity"); // switch back to chat to show Jules' response
  ...
  const pri = useMemo(() => getPRInfo(session, activities), [session, activities]);
  const pr = pri;
  const isActuallyDone = activities.some(a => a.sessionCompleted);
  const currentState = isActuallyDone ? "COMPLETED" : session.state;
  const isFinished = currentState === "COMPLETED" || currentState === "FAILED";
  console.log("isFinished in SessionDetail:", isFinished);
  const m       = STATUS_META[currentState] || STATUS_META.QUEUED;
  ...
  useEffect(() => {
    if (currentState === "AWAITING_PLAN_APPROVAL" && latestPlan) setTab("plan");
  }, [currentState, latestPlan]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:0,position:"relative"}}>
      {/* ── Header ── */}
      <div style={{
        padding:scrolled?"6px 12px 6px":"12px 16px 0",
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:scrolled?0:6}}>
          {!isDesktop&&(
          ...
          <div style={{flex:1,minWidth:0,transition:"all .2s ease"}}>
            <div style={{
              fontFamily:"'IBM Plex Sans',sans-serif",
              ...
              display: scrolled ? "block" : "-webkit-box",
            ...
            {scrolled && (
              <div style={{display:"flex", alignItems:"center", gap:8, marginTop: 2, animation:"fadeIn .2s ease"}}>
                 {repo && (
                 ...
                 {session.sourceContext?.source && (
                   <a href={b?.repoUrl ? `${b.repoUrl}/branches` : "#"} target="_blank" rel="noopener" style={{display:"flex", alignItems:"center", gap:3, color:T.blue, textDecoration:"none", flexShrink:0, borderBottom:`1px solid ${T.blue}40`}}>
                 ...
                 {pr && (
                  ...
                  {checks && (
                    <div style={{
                      background:checks.state==="success"?T.brandDim:checks.state==="failure"?T.redDim:T.amberDim,
                      border:`1px solid ${checks.state==="success"?T.brand:checks.state==="failure"?T.red:T.amber}40`,
                      borderRadius:3, padding:"1px 4px", display:"flex", alignItems:"center", gap:3,
                      color:checks.state==="success"?T.brandLight:checks.state==="failure"?T.red:T.amber,
                      fontFamily:"'JetBrains Mono',monospace", fontSize:8, fontWeight:900, flexShrink:0
                    }}>
                      <Ic n={checks.state==="success"?"check":checks.state==="failure"?"x":"refresh"} s={9} c={checks.state==="success"?T.brandLight:checks.state==="failure"?T.red:T.amber}/>
                    </div>
                  )}
              </div>
            )}
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
            ...
            <div style={{position: "relative"}}>
              <button onClick={handleRefresh} disabled={busy} title="Refresh activity" style={{
                background:justUpdated?T.brand:isStale?T.brand:T.surfaceHi,
                border:`1px solid ${justUpdated?T.brand:isStale?T.brand:T.borderHi}`,
                borderRadius:4, cursor:busy?"default":"pointer", display:"flex", padding:4,
                opacity:busy?0.5:1,
                transition: "all .3s"
              }}>
                <div style={{ display: "flex", animation: busy ? "spin 1s linear infinite" : "none" }}>
                  <Ic n={justUpdated?"check":"refresh"} s={14} c={busy?T.brand:(justUpdated||isStale?"#000":T.dim)}/>
                </div>
              </button>
              {justUpdated && (
                <div style={{
                  position: "absolute", top: -20, left: "50%", transform: "translateX(-50%)",
                  background: T.brand, color: "#000", fontSize: 8, fontWeight: 900,
                  padding: "2px 6px", borderRadius: 4, fontFamily: "'JetBrains Mono',monospace",
                  whiteSpace: "nowrap", animation: "slideUp .3s ease-out"
                }}>UPDATED</div>
              )}
            </div>
            <Pill status={currentState} small hideLabel={scrolled}/>

            {/* Desktop-only side buttons in Row 1 */}
            {isDesktop && (
              <>
                <button
                  ...
                  style={{
                    background:isArchived ? T.purpleDim : T.surfaceHi,
                    border:`1px solid ${isArchived ? T.purple+"40" : T.borderHi}`,
                    borderRadius:4, cursor:"pointer", display:"flex", alignItems:"center", padding:4, transition:"all .2s"
                  }}
                >
                  <Ic n={isArchived ? "unarchive" : "archive"} s={14} c={isArchived ? T.purple : T.muted}/>
                </button>
                <button onClick={() => { if(confirm("Delete this session?")) handleDelete(); }} disabled={busy} title="Delete session" style={{background:T.redDim, border:`1px solid ${T.red}30`, borderRadius:4, cursor:"pointer", display:"flex", alignItems:"center", padding:4, transition:"opacity .2s"}}>
        ...
        {/* Row 2: Secondary Metadata & Actions (Hidden when scrolled on mobile) */}
        {!scrolled && (
          <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:8, overflowX:"auto", scrollbarWidth:"none", WebkitOverflowScrolling:"touch", animation:"fadeIn .2s ease"}}>
            {(!isFinished || isSyncing || syncPhase === "DONE") && (
              <div style={{
                display:"flex", alignItems:"center", gap:6,
                background: isSyncing ? `${T.amber}15` : (syncPhase === "DONE" ? `${T.brand}20` : T.brandDim),
                border: `1px solid ${isSyncing ? T.amber : (syncPhase === "DONE" ? T.brand : T.brandDark) + "40"}`,
                borderRadius:4, padding:"2px 8px", flexShrink:0,
                transition: "all .3s ease"
              }}>
                 <div style={{
                   width:6, height:6, borderRadius:"50%",
                   background: isSyncing ? T.amber : T.brandLight,
                   animation: isSyncing ? (syncPhase === "CACHE" ? "pulse 1s infinite" : "spin 1s linear infinite") : "pulse 2s infinite"
                 }}/>
                 <span style={{fontFamily:"'JetBrains Mono',monospace", fontSize:9, fontWeight:800, color: isSyncing ? T.amber : T.brand}}>
                   {isSyncing ? (
                     syncPhase === "CACHE" ? "RECOVERING CACHE..." :
                     syncPhase === "FETCH" ? `FETCHING (${syncStats.count} ITEMS)...` :
                     syncPhase === "SYNC" ? "RECONCILING..." : "SYNCING..."
                   ) : `UP TO DATE · ${activities.length} ITEMS`}
                 </span>
              </div>
            )}
            ...
            {checks && (
              <div style={{
                display:"flex", alignItems:"center", gap:4, background:checks.state==="success"?T.brandDim:checks.state==="failure"?T.redDim:T.amberDim,
                border:`1px solid ${checks.state==="success"?T.brand:checks.state==="failure"?T.red:T.amber}40`,
                borderRadius:4, padding:"3px 8px", color:checks.state==="success"?T.brandLight:checks.state==="failure"?T.red:T.amber,
                fontFamily:"'JetBrains Mono',monospace", fontSize:10, fontWeight:900, cursor:checks.url?"pointer":"default", flexShrink:0,
                animation: isSyncing ? "shimmerPulse 1.5s infinite 0.2s" : "none"
              }} onClick={() => checks.url && window.open(checks.url, "_blank")}>
                <Ic n={checks.state==="success"?"check":checks.state==="failure"?"x":"refresh"} s={10} c={checks.state==="success"?T.brandLight:checks.state==="failure"?T.red:T.amber}/>
                {checks.label}
              </div>
            )}

            {session.sourceContext?.source && (
              <a href={b?.repoUrl ? `${b.repoUrl}/branches` : "#"} target="_blank" rel="noopener" style={{
                ...
                {b?.working || "main"}
                {ahead > 0 && <span style={{fontSize:9}}>+{ahead}</span>}
              </a>
            )}

            {session.url && (
              <a href={session.url} target="_blank" rel="noopener" style={{
              ...
              <div style={{display:"flex", gap:6, marginLeft:"auto"}}>
                <button
                  ...
                  style={{
                    background:isArchived ? T.purpleDim : T.surfaceHi,
                    border:`1px solid ${isArchived ? T.purple+"40" : T.borderHi}`,
                    borderRadius:4, cursor:"pointer", display:"flex", alignItems:"center", padding:5, transition:"all .2s"
                  }}
                >
                  <Ic n={isArchived ? "unarchive" : "archive"} s={13} c={isArchived ? T.purple : T.muted}/>
                </button>
                <button onClick={() => { if(confirm("Delete this session?")) handleDelete(); }} disabled={busy} title="Delete session" style={{background:T.redDim, border:`1px solid ${T.red}30`, borderRadius:4, cursor:"pointer", display:"flex", alignItems:"center", padding:5}}>
      ...
      <div style={{
          height:scrolled?0:2, overflow:"hidden", transition:"all .25s ease",
          marginBottom:scrolled?0:5, opacity:scrolled?0:1,
          pointerEvents:scrolled?"none":"auto",
        }}>
          <Bar pct={pct} status={currentState}/>
        </div>
        ...
        <div style={{
          ...
          overflow:"hidden",
          fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:T.dim,flexWrap:"nowrap",alignItems:"center",
          transition:"all .25s ease",
          pointerEvents:scrolled?"none":"auto",
        }}>
          <span>ID: {session.id?.slice(0,10)}</span>
          {repo && (
            <span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap", display:"flex", alignItems:"center", gap:4, fontWeight:600}}>
              <Ic n="code" s={9} c={T.dim}/>
              {repo}
            </span>
          )}
        ...
        {(pr?.state === "closed" || !pr) && b?.isNew && ahead > 0 && (
          <div style={{
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontFamily:"'IBM Plex Sans',sans-serif", fontSize:13, color:T.textDim}}>
                {ahead} commit{ahead!==1?'s':''} pushed to <span style={{color:T.blue, fontWeight:600}}>{b.working}</span>. No PR created yet.
              </div>
            </div>
            {b.repoUrl && (
              <div style={{display:"flex", gap:8}}>
                <a href={`${b.repoUrl}/branches`} target="_blank" rel="noopener" style={{
                ...
                <a href={`${b.repoUrl}/compare/${b.base}...${b.working}`} target="_blank" rel="noopener" style={{
        ...
        {summary && (
          <div style={{
            ...
            borderRadius:8, transition:"all .25s ease",
            pointerEvents:scrolled?"none":"auto",
            boxShadow:"none",
          }}>
            <div style={{
        ...
        <div style={{
          ...
          <div style={{
            height: "100%",
            width: isSyncing ? `${Math.min(98, (syncStats.pages / (activityLimit / 50)) * 100)}%` : "100%",
            background: isSyncing ? `linear-gradient(90deg, ${T.amber}, ${T.brand})` : T.brand,
            boxShadow: `0 0 10px ${isSyncing ? T.amber : T.brand}80`,
        ...
        <div style={{display:"flex", paddingBottom:scrolled?4:0, transition:"all .2s ease"}}>
          {TABS.map(t=>(
      ...
      {/* ── Content ── */}
      <div ref={contentRef} onScroll={handleScroll} style={{flex:1,overflowY:"auto",padding:"14px 16px",paddingBottom:isDesktop?80:220,WebkitOverflowScrolling:"touch",minHeight:200,position:"relative"}}>
        {tab==="activity"&&(
          <>
            <div style={{display:"flex", alignItems:"center", gap:6, marginBottom:16, flexWrap:"wrap"}}>
              {["ALL", "MESSAGES", "REVIEWS", "SYSTEM"].map(f => (
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer", outline: chatFilter === f ? `1px solid ${T.brand}40` : `1px solid ${T.border}`,
                    transition: "all .15s"
                  }}
                >
                  {f}
              ...
              <button
                onClick={() => {
                  ...
                  alert("Copied filtered chat to clipboard");
                }}
                style={{
                ...
                return (
                  <div key={getActKey(act)} style={{
                    ...
                    <div style={{marginTop:12, display:"flex", alignItems:"center", justifyContent:"space-between"}}>
                       ...
                       <button
                         onClick={() => { navigator.clipboard.writeText(p.description); alert("Copied review text"); }}
                         style={{
                           background:"transparent", border:"none", color:T.brand,
                           fontFamily:"'JetBrains Mono',monospace", fontSize:11, fontWeight:700,
                           cursor:"pointer", display:"flex", alignItems:"center", gap:4
            ...
            <div key="original">
              ...
              <div style={{
                ...
                <div style={{ position:"absolute", top:12, right:12, display:"flex", gap:6 }}>
                  <button onClick={() => onEditMessage(session.prompt)} style={{
                    background:T.dim, border:"none", borderRadius:4, padding:"4px 8px",
            ...
            {activities.filter(a => a.userMessaged).map((a, i) => (
              <div key={a.id || i}>
                <div style={{
                  ...
                  FOLLOW-UP #{i+1}
                  <div style={{height:1, flex:1, background:T.brandDim}}/>
                  <span style={{color:T.textDim, fontSize:10}}>{fmtTime(new Date(a.createTime).getTime())}</span>
                </div>
                <div style={{
                  background:T.surfaceHi, border:`1px solid ${T.border}`,
                  borderRadius:10, padding:"20px", boxShadow:"none", position:"relative"
                  ...
                  <div style={{ position:"absolute", top:12, right:12, display:"flex", gap:6 }}>
                    <button onClick={() => onEditMessage(a.userMessaged.userMessage)} style={{
                      background:T.dim, border:"none", borderRadius:4, padding:"4px 8px",
                      color:T.brand, fontFamily:"'JetBrains Mono',monospace", fontSize:10, fontWeight:700, cursor:"pointer"
                    }}>EDIT</button>
                  </div>
        ...
        {tab==="media"&&(
          ...
          :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:12}}>
            {mediaArtifacts.map((m,i)=>{
              const isVideo = (m.mimeType || "").includes("video") || (m.mimeType || "").includes("webm");
              return (
                <button
                  key={i}
                ...
                >
                  <div style={{ width:"100%", height:120, background:"#040507", display:"flex", alignItems:"center", justifyContent:"center" }}>
                    {isVideo ? (
                      <video src={`data:${m.mimeType};base64,${m.data}`} muted autoPlay loop playsInline
                        style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                    ) : (
      ...
      {canSend&&(
        <div style={{
          ...
          <MultiPersonaPicker personas={personas} selectedIds={selectedPersonas} onToggle={togglePersona} style={{marginBottom:10}} />
          ...
          <div style={{position:"relative", display:"flex", flexDirection:"column"}}>
            <textarea
              ...
              style={{
                ...
                resize:"none", outline:"none", transition:"all .2s ease",
            ...
            <div style={{
              ...
              {msg.trim() && (
                <button onClick={()=>setMsg("")} title="Clear" style={{background:"transparent", border:"none", padding:4, cursor:"pointer", display:"flex", alignItems:"center"}}>
                  <Ic n="x" s={16} c={T.red}/>
                </button>
              )}
              <button onClick={()=>setExpanded(!expanded)} title={expanded ? "Minimize" : "Expand"} style={{background:"transparent", border:"none", padding:4, cursor:"pointer", display:"flex", alignItems:"center"}}>
              ...
              <button
                ...
                style={{
                  ...
                  opacity: msg.trim()&&!busy ? 1 : 0.4, transition: "all .2s"
                }}
              >
      ...
      <div style={{
        ...
        bottom: (isDesktop || composerVisible) ? (isDesktop ? 110 : 240) : 30,
...
const REPO_STATS_KEY = "jac_repo_stats";
const LAST_SOURCE_KEY = "jac_last_source";
const ACT_MAP_KEY = "jac_act_map";
const ACT_STATS_KEY = "jac_act_stats";
const SESSION_CACHE_KEY = "jac_session_cache";

function cleanupActivityStats(validIds) {
  try {
    const stats = JSON.parse(localStorage.getItem(ACT_STATS_KEY) || "{}");
    ...
    if (changed) {
      localStorage.setItem(ACT_STATS_KEY, JSON.stringify(stats));
...
function loadRepoStats() {
  try { return JSON.parse(localStorage.getItem(REPO_STATS_KEY)) || {}; } catch { return {}; }
}
function incRepoStat(name) {
  if (!name) return;
  try {
    const stats = loadRepoStats();
    stats[name] = (stats[name] || 0) + 1;
    localStorage.setItem(REPO_STATS_KEY, JSON.stringify(stats));
  } catch {}
}
function loadLastSource() {
  try { return localStorage.getItem(LAST_SOURCE_KEY) || ""; } catch { return ""; }
}
function saveLastSource(name) {
  try { localStorage.setItem(LAST_SOURCE_KEY, name || ""); } catch {}
}
...
const NewSession = ({ apiKey, personas, onBack, onCreate, isDesktop, plan, todayCount }) => {
  ...
  const payloadEst = (JSON.stringify({prompt,source,branch}).length/1024).toFixed(1);
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:0,position:"relative"}}>
      {showConfirm && (
        <div style={{
          ...
          <div style={{
            width:"100%", maxWidth:420, background:T.surface, border:`1px solid ${T.borderHi}`,
            borderRadius:16, padding:28, boxShadow:"0 20px 50px rgba(0,0,0,0.6)",
            ...
            <div style={{textAlign:"center", marginBottom:24}}>
              <div style={{
                ...
                <Ic n="tasks" s={22} c={T.brand}/>
              </div>
              <div style={{fontFamily:"'JetBrains Mono',monospace", fontSize:16, fontWeight:900, color:T.text, letterSpacing:"0.05em"}}>CONFIRM SESSION</div>
              <div style={{fontFamily:"'IBM Plex Sans',sans-serif", fontSize:14, color:T.muted, marginTop:4}}>Ready to assign this task to Jules?</div>
            </div>
            ...
            <div style={{display:"flex", flexDirection:"column", gap:16, marginBottom:28}}>
              <div style={{
                ...
                animation: (showSources || showBranches) ? "none" : "pulseRepo 1s infinite ease-in-out",
                transition: "all .2s"
              }}>
                ...
                <div style={{position:"relative"}}>
                  <input
                    value={sourceSearch}
                    onChange={e=>{setSourceSearch(e.target.value); setShowSources(true);}}
                    onFocus={()=>setShowSources(true)}
                    placeholder="Search repositories..."
                    style={{...inputSt, paddingRight:36, fontSize:13, padding: "8px 10px"}}
                  />
                  <div style={{position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", display:"flex", alignItems:"center", pointerEvents:"none"}}>
                    ...
                    <>
                      ...
                      <div style={{
                        ...
                        {filteredSources.map((s, idx) => {
                          const isRepoless = s === null;
                          const sName = isRepoless ? "" : s.name;
                          const isSelected = sName === source;
                          return (
                            <button
                              key={isRepoless ? "_repoless" : s.name}
                              onClick={()=>{setSource(sName); setSourceSearch(getSourceDisplay(s)); setShowSources(false);}}
                              style={{
                                width:"100%", padding:"10px 12px", background:"none", border:"none",
                                textAlign:"left", cursor:"pointer", display:"flex", alignItems:"center", gap:8,
              ...
              <div style={{background:T.surfaceHi, borderRadius:10, padding:16, border:`1px solid ${T.border}`}}>
                <div style={{fontFamily:"'JetBrains Mono',monospace", fontSize:10, color:T.purple, fontWeight:800, letterSpacing:"0.1em", marginBottom:8}}>PROMPT SUMMARY</div>
            ...
            <div style={{display:"flex", gap:12}}>
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow:`0 8px 24px ${T.brand}40`, transition:"transform .2s"
                }}
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 20px",WebkitOverflowScrolling:"touch",minHeight:0}}>
        <div style={{maxWidth:560,margin:"0 auto"}}>
          {err&&<div style={{padding:"8px 12px",borderRadius:5,marginBottom:12,background:T.redDim,border:`1px solid ${T.red}40`,fontFamily:"'JetBrains Mono',monospace",fontSize:12,color:T.red}}>{err}</div>}

          <Field label="TASK PROMPT *" style={{marginBottom:12}}>
            <div style={{ position: "relative" }}>
              ...
              <div style={{
                ...
                {prompt.trim() && (
                   <button onClick={()=>setPrompt("")} title="Clear Prompt" style={{background:"transparent", border:"none", padding:4, cursor:"pointer", display:"flex", alignItems:"center"}}>
                     <Ic n="x" s={14} c={T.red}/>
                   </button>
                )}
                <button onClick={()=>setExpanded(!expanded)} title={expanded?"Minimize":"Expand"} style={{background:"transparent", border:"none", padding:4, cursor:"pointer", display:"flex", alignItems:"center"}}>
          ...
          <Btn onClick={() => setShowConfirm(true)} disabled={!prompt.trim()||submitting||loadingSrc||(plan&&todayCount.total>=plan.daily)} style={{width:"100%"}}>
                    ...
                    <div key={r.id} style={{display:"flex",gap:12,alignItems:"center",padding:"12px 16px",borderBottom:i<log.length-1?`1px solid ${T.border}`:"none"}} role="listitem">
                      ...
                      <div style={{display:"flex", alignItems:"center", gap:8}}>
                        <button
                          ...
                          style={{background:T.surface, border:`1px solid ${T.border}`, borderRadius:4, cursor:"pointer", padding:6, display:"flex"}}
                        >
                          <Ic n="copy" s={12} c={T.muted}/>
                        </button>
                      </div>
                    </div>
                  ))
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:0}}>
      <div style={{padding:"12px 16px 0",borderBottom:`1px solid ${T.border}`,background:T.surface,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          ...
          {isDesktop && (
            <button onClick={onBack} title="Close Settings" style={{background:"none",border:"none",cursor:"pointer",display:"flex",padding:4,opacity:0.8}}>
              <Ic n="x" s={18} c={T.text}/>
            </button>
          )}
        </div>
          <div style={{display:"flex", gap:16, position:"relative", borderBottom: `1px solid ${T.border}`, overflowX: "auto", scrollbarWidth: "none", WebkitOverflowScrolling: "touch"}}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              fontSize:11, fontWeight:tab===t.id?800:600, letterSpacing:"0.05em", transition:"all .2s",
              display:"flex", alignItems:"center", gap:6,
            }}
              ...
              <div style={{background:T.surfaceHi, padding:20, borderRadius:12, border:`1px solid ${T.border}`}}>
                ...
                {planId==="custom" && (
                  <div style={{display:"flex", alignItems:"center", gap:10, marginBottom:16, background:T.surface, padding:12, borderRadius:8, border:`1px solid ${T.border}`}}>
                    <span style={{fontFamily:"'JetBrains Mono',monospace", fontSize:11, color:T.textDim, fontWeight:700}}>DAILY LIMIT:</span>
                    <input
                      type="number"
                      value={customDaily}
                ...
                <div style={{background:T.surface, padding:16, borderRadius:8, border:`1px solid ${T.border}`}}>
                  ...
                  <div style={{marginTop:12,fontSize:11,color:T.textDim,fontFamily:"'IBM Plex Sans',sans-serif",lineHeight:1.6, opacity:0.8}}>
                    <div style={{marginBottom:6}}>
                      ...
                      {todayCount.nextResetTs && (
                        <div style={{fontFamily:"'JetBrains Mono',monospace", fontSize:11, color:T.amber, marginTop:4, fontWeight:700}}>
                          NEXT SLOT RECOVERY: {fmtTime(todayCount.nextResetTs)} ({todayCount.resetIn})
                        </div>
                      )}
                    </div>
                    <div>{plan.model} · {plan.concurrent} concurrent max</div>
                    {todayCount.total>=plan.daily && <div style={{color:T.red,marginTop:6,fontWeight:600, borderTop:`1px solid ${T.red}20`, paddingTop:6}}>⛔ Daily limit reached. Next recovery at {fmtTime(todayCount.nextResetTs)}.</div>}
                  </div>

                  {todayCount.upcomingResets?.length > 0 && (
                    <div style={{marginTop:20, borderTop:`1px solid ${T.border}`, paddingTop:16}}>
                      <div style={{fontFamily:"'JetBrains Mono',monospace", fontSize:10, color:T.dim, fontWeight:800, letterSpacing:"0.1em", marginBottom:12}}>UPCOMING SLOT RECOVERIES</div>
                      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
                        {todayCount.upcomingResets.map((r, i) => (
                          <div key={i} style={{background:T.surfaceHi, border:`1px solid ${T.border}`, borderRadius:6, padding:"8px 10px", display:"flex", flexDirection:"column", gap:2}}>
                            <div style={{fontFamily:"'JetBrains Mono',monospace", fontSize:11, color:T.text, fontWeight:700}}>{fmtTime(r.ts)}</div>
                            <div style={{fontFamily:"'JetBrains Mono',monospace", fontSize:9, color:T.amber, fontWeight:800}}>IN {r.in}</div>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {tab === "personas" && (
            <div style={{animation:"fadeIn .2s ease", display:"flex", flexDirection:"column", gap:16}}>
              <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4}}>
                ...
                <button onClick={() => { if(confirm("Reset all personas to defaults?")) { setPersonas(resetPersonas()); } }} style={{background:"none", border:"none", cursor:"pointer", color:T.red, fontFamily:"'JetBrains Mono',monospace", fontSize:10, fontWeight:700}}>RESET ALL</button>
              </div>
              {personas.map(p => (
                <div key={p.id} style={{background:T.surfaceHi, border:`1px solid ${T.border}`, borderRadius:10, padding:14}}>
                  <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:10}}>
                    ...
                    <span style={{flex:1}}/>
                    <button onClick={() => {
                      const next = prompt(`Edit prompt for ${p.label}:`, p.prompt);
                      if (next !== null) {
                        savePersonaPrompt(p.id, next);
                        setPersonas(loadPersonas());
                      }
                    }} style={{background:T.dim, border:"none", borderRadius:4, padding:"3px 8px", cursor:"pointer", color:T.brand, fontFamily:"'JetBrains Mono',monospace", fontSize:10, fontWeight:700}}>EDIT PROMPT</button>
                  </div>
                  <div style={{fontSize:13, color:T.textDim, lineHeight:1.5, fontFamily:"'IBM Plex Sans',sans-serif"}}>{p.prompt}</div>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{animation:"fadeIn .3s cubic-bezier(0.4, 0, 0.2, 1)", display:"flex", flexDirection:"column", gap:24}}>
              ...
              <div style={{background:"rgba(255,85,102,0.03)", border:`1px dashed ${T.red}30`, borderRadius:12, padding:24, textAlign:"center"}}>
                ...
                <button onClick={resetApp} style={{
                  ...
                  boxShadow:`0 8px 24px ${T.red}40`, transition:"transform .2s"
...
const SessionList = ({ sessions, onSelect, onRefresh, refreshing, justRefreshed, selectedId, isDesktop, onNew,  onSettings, pollInterval, sessionLimit, countdown, plan, todayCount, searchQuery, setSearchQuery, archivedIds, showArchived, setShowArchived, activitiesMap = {}, activityStatsMap = {}, error, clearError, isBoosted, readMap }) => {
  const [filter,setFilter] = useState("ALL");
  const [scrolled, handleScroll] = useScrollThreshold();
  const [searchOpen, setSearchOpen] = useState(false);
  const baseFiltered = useMemo(() => {
    let list = sessions.filter(s => archivedIds.has(s.id) === showArchived);
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase();
      list = list.filter(s => (s.title||"").toLowerCase().includes(q) || (s.prompt||"").toLowerCase().includes(q) || (s.id||"").toLowerCase().includes(q));
    }
    return list;
  }, [sessions, archivedIds, showArchived, searchQuery]);
  const filtered = useMemo(() => filter==="ALL" ? baseFiltered : baseFiltered.filter(s=>s.state===filter), [filter, baseFiltered]);
  const active = useMemo(() => sessions.filter(s=>ACTIVE_STATES.has(s.state)).length, [sessions]);

  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:0}}>
      <div style={{padding:scrolled?"8px 16px 8px":"12px 16px 0",background:T.surface,borderBottom:`1px solid ${T.border}`,flexShrink:0,transition:"padding .2s ease-out, background .2s ease-out",zIndex: 5, contain: "layout"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:scrolled?0:10}}>
          <div style={{width:scrolled?20:32,height:scrolled?20:32,borderRadius:6,background:T.brand,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:scrolled?11:18,fontWeight:900,color:"#000",boxShadow:scrolled?"none":`0 0 12px ${T.brandDark}40`,flexShrink:0,transition:"all .2s ease"}}>J</div>
          <div style={{minWidth:0, transition:"all .2s ease"}}>
          ...
          <div style={{marginLeft:"auto",display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
            <button onClick={() => setSearchOpen(!searchOpen)} title="Search sessions" style={{width:30,height:30,borderRadius:5,background:searchOpen||searchQuery?T.blueDim:"transparent",border:`1px solid ${searchOpen||searchQuery?T.blue+"40":T.border}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Ic n="search" s={15} c={searchOpen||searchQuery?T.blue:T.muted}/></button>
            {isDesktop&&<button onClick={onNew} title="New Session" style={{width:30,height:30,borderRadius:5,background:T.brandDim,border:`1px solid ${T.brand}40`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Ic n="plus" s={15} c={T.brand}/></button>}
            <button onClick={onSettings} title="Settings" style={{width:30,height:30,borderRadius:5,background:T.surfaceHi,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Ic n="settings" s={15} c={T.muted}/></button>
            <div style={{display:"flex", alignItems:"center", gap:4}}>
              ...
              <button
                ...
                style={{
                  ...
                  display:"flex", padding:4, borderRadius:20, transition:"all .15s",
                ...
                {countdown > 0 && !refreshing && (
                  <svg style={{ position: "absolute", inset: 0, transform: "rotate(-90deg)", width: "100%", height: "100%", pointerEvents: "none" }}>
                    <circle
                      ...
                      style={{ transition: "stroke-dashoffset 1s linear", opacity: 0.35 }}
                    />
                  </svg>
                )}
        ...
        {error && (
          <div style={{
            ...
            <button onClick={clearError} style={{background:"none", border:"none", cursor:"pointer", padding:2}}><Ic n="x" s={12} c={T.red}/></button>
          </div>
        )}
        {(searchOpen || searchQuery) && (
          <div style={{height:scrolled?0:42, overflow:"hidden", opacity:scrolled?0:1, transition:"all .25s ease", marginBottom:scrolled?0:8, padding:scrolled?0:"4px 2px", pointerEvents:scrolled?"none":"auto"}}>
            <div style={{position:"relative", display:"flex", alignItems:"center"}}>
              ...
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by title, prompt or ID..." style={{...inputSt, paddingLeft:34, paddingRight:34, borderColor:T.blue+"40", background:T.surfaceHi}} autoFocus />
              {searchQuery && <button onClick={() => setSearchQuery("")} style={{position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", padding:4}}><Ic n="x" s={14} c={T.muted}/></button>}
            </div>
          </div>
        )}
        {isDesktop && (
          <div style={{display:"flex", gap:4, alignItems:"center", height:scrolled?0:22, overflow:"hidden", opacity:scrolled?0:1, marginBottom:scrolled?0:8, padding:scrolled?0:"4px 2px", transition:"all .25s ease", pointerEvents:scrolled?"none":"auto"}}>
            ...
            <button onClick={() => setShowArchived(false)} style={{flexShrink:0, padding:"3px 9px", borderRadius:20, border:"none", background:!showArchived ? T.brandDim : "transparent", outline:`1px solid ${!showArchived ? T.brand+"60" : T.border}`, color:!showArchived ? T.brand : T.muted, fontFamily:"'JetBrains Mono',monospace", fontSize:11, fontWeight:!showArchived?700:400, letterSpacing:"0.05em", cursor:"pointer", transition:"all .12s"}}>ACTIVE</button>
            <button onClick={() => setShowArchived(true)} style={{flexShrink:0, padding:"3px 9px", borderRadius:20, border:"none", background:showArchived ? T.purpleDim : "transparent", outline:`1px solid ${showArchived ? T.purple+"60" : T.border}`, color:showArchived ? T.purple : T.muted, fontFamily:"'JetBrains Mono',monospace", fontSize:11, fontWeight:showArchived?700:400, letterSpacing:"0.05em", cursor:"pointer", transition:"all .12s"}}>ARCHIVED</button>
          </div>
        )}
        <div style={{position: "relative", maxHeight:scrolled?0:60, opacity:scrolled?0:1, transition:"all .25s ease", pointerEvents:scrolled?"none":"auto"}}>
          ...
          <div style={{overflowX:"auto", padding:scrolled?0:"4px 2px 10px", scrollbarWidth:"none", WebkitOverflowScrolling:"touch"}}>
            <div style={{display:"flex",gap:5,minWidth:"max-content",padding:"4px 0", alignItems:"center"}}>
              ...
              {FILTERS.map(f => {
                ...
                return (
                  <button key={f} onClick={()=>setFilter(f)} style={{flexShrink:0,padding:"4px 10px",borderRadius:20,border:"none",background:isAct?`${ac}20`:"transparent",outline:`1px solid ${isAct?`${ac}80`:T.border}`,color:isAct?ac:T.muted,fontFamily:"'JetBrains Mono',monospace",fontSize:11,fontWeight:700,letterSpacing:"0.06em",cursor:"pointer",transition:"background .12s, color .12s"}}>{FILTER_LABELS[f]||f} {cnt>0?cnt:""}</button>
                );
              })}
            </div>
          </div>
      ...
      <div onScroll={handleScroll} style={{flex:1,overflowY:"auto",padding:"10px 12px",WebkitOverflowScrolling:"touch",minHeight:200}}>
        ...
        {!refreshing&&filtered.length===0&&<div style={{textAlign:"center",padding:"50px 0",fontFamily:"'JetBrains Mono',monospace",fontSize:13,color:T.textDim}}>{filter==="ALL"?"No sessions yet":"No sessions with this status"}</div>}
        {filtered.map((s,i)=><SessionCard key={s.id||s.name} index={i+1} s={s} onPress={()=>onSelect(s)} isSelected={s.id===selectedId} activities={activitiesMap[s.id] || []} stats={activityStatsMap[s.id]} lastReadTs={readMap[s.id]}/>)}
      </div>
    </div>
  );
};
...
function JulesClient() {
  const [apiKey,setApiKey]     = useState(() => { try { return localStorage.getItem("jac_key")||""; } catch (e) { return ""; } });
  const [sessions,setSessions] = useState([]);
  const [activitiesMap, setActivitiesMap] = useState(() => {
    try { return JSON.parse(localStorage.getItem(ACT_MAP_KEY)) || {}; } catch { return {}; }
  });
  const [activityStatsMap, setActivityStatsMap] = useState(() => {
    try { return JSON.parse(localStorage.getItem(ACT_STATS_KEY)) || {}; } catch { return {}; }
  });
  ...
  const allSessions = useMemo(() => {
    const map = new Map();
    [...sessions, ...supplementalSessions].forEach(s => map.set(s.id, s));
    return Array.from(map.values()).sort((a, b) => {
      const aTime = new Date(a.updateTime || a.createTime).getTime();
      const bTime = new Date(b.updateTime || b.createTime).getTime();
      return bTime - aTime;
    });
  ...
  const [sessionRegistry, setSessionRegistry] = useState(() => {
    try { return JSON.parse(localStorage.getItem("jac_session_registry") || "{}"); }
    catch { return {}; }
  });
  ...
  const todayCount = useMemo(() => {
    ...
    const prCreated = sessions.filter(s => windowIds.has(s.id) && !!getPR(s)).length;

    return { total: allKnown.length, done: prCreated, resetIn, nextResetTs, upcomingResets };
  }, [sessions, sessionRegistry]);
  const lastFetchTime = useRef(null);

  const [archivedIds, setArchivedIds] = useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem("jac_archived") || "[]")); }
  ...
  const [readMap, setReadMap] = useState(() => {
    try { return JSON.parse(localStorage.getItem("jac_read_map") || "{}"); }
    catch { return {}; }
  });
  ...
  const setCustomDaily = useCallback(v => { saveCustomDaily(v); setCustomDailyRaw(v); }, []);
  const plan = useMemo(() => {
  ...
  useEffect(() => {
    const handleStorage = (e) => {
      ...
      if (e.key === "jac_archived") {
        try { setArchivedIds(new Set(JSON.parse(e.newValue || "[]"))); } catch {}
      }
      if (e.key === ACT_STATS_KEY) {
        try { setActivityStatsMap(JSON.parse(e.newValue || "{}")); } catch {}
      }
    };
  ...
  useEffect(() => {
    try { localStorage.setItem("jac_archived", JSON.stringify(Array.from(archivedIds))); }
    catch {}
  }, [archivedIds]);

  useEffect(() => {
    try { localStorage.setItem("jac_read_map", JSON.stringify(readMap)); }
    catch {}
  }, [readMap]);

  useEffect(() => {
    try { localStorage.setItem("jac_session_registry", JSON.stringify(sessionRegistry)); }
    catch {}
  }, [sessionRegistry]);
  ...
  const fetchSessions = useCallback(async (quiet=false, forceFull=false) => {
    ...
    try {
      ...
      do {
        ...
        const d = await apiCall(apiKey, `/sessions?${qs}`, { _label:`Sessions ${isFull?"full":"Δ"} p${iterations+1}` });
        ...
        const hasOlderThanWindow = batch.length > 0 && new Date(batch[batch.length-1].createTime) < windowStart;
      ...
      setSessions(prev => {
        ...
        if (isFull) {
          ...
          merged = [
            ...incoming,
            ...prev.filter(s => !incomingIds.has(s.id))
          ];
        }

        // Sort: Active first, then by latest update/create time
  ...
  const handleDelete = useCallback(id => {
    ...
    try { localStorage.removeItem(`jac_draft_${id}`); } catch {}
    setSelected(null);
    setDesktop("empty");
    setMobile("list");
  }, []);
  ...
  const saveKey = k => {
    try { localStorage.setItem("jac_key", k); } catch {}
    lastFetchTime.current = null;
    setApiKey(k);
  };
    ...
    return (
      <Shell desktop>
        ...
        <div style={{width:320,flexShrink:0,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden"}}>
          <SessionList
            sessions={sessions} onSelect={handleSelect} onRefresh={()=>fetchSessions(false, true)}
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden",minWidth:0}}>
          ...
          {desktopView==="new"&&(
            <NewSession apiKey={apiKey} personas={personas} onBack={()=>setDesktop("empty")} onCreate={handleCreate} isDesktop plan={plan} todayCount={todayCount}/>
          )}
          ...
          {desktopView==="network"&&( <NetworkMonitor onBack={()=>setDesktop("empty")} isDesktop/> )} {desktopView==="settings"&&(
            <SettingsView onBack={()=>setDesktop("empty")} isDesktop
              ...
              actPollInterval={actPollInterval} setActPollInterval={setActPollInterval}
              activePollInterval={activePollInterval} setActivePollInterval={setActivePollInterval}
              sessionLimit={sessionLimit} setSessionLimit={setSessionLimit}
              activityLimit={activityLimit} setActivityLimit={setActivityLimit}
              cacheLimit={cacheLimit} setCacheLimit={setCacheLimit}
              personas={personas} setPersonas={setPersonas}
              planId={planId} setPlanId={setPlanId}
              customDaily={customDaily} setCustomDaily={setCustomDaily}
              plan={plan} todayCount={todayCount} />
          )}
        </div>
      </Shell>
  ...
  return (
    <Shell>
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",minHeight:0}}>
        {mobileScreen==="list"&&(
          <SessionList sessions={allSessions} onSelect={handleSelect} onRefresh={()=>fetchSessions(false, true)}
        ...
        {mobileScreen==="new"&&(
          <NewSession apiKey={apiKey} personas={personas} onBack={()=>setMobile("list")} onCreate={s=>{ setSessions(prev=>[s,...prev]); setMobile("list"); }} isDesktop={false} plan={plan} todayCount={todayCount}/>
        )}
      ...
      <nav style={{display:"flex",borderTop:`1px solid ${T.border}`,background:T.surface,transform:"translateZ(0)",flexShrink:0}}>
        {[
          ...
          {id:"new",     n:"plus",  label:"NEW", onClick:() => setMobile("new")},
